...
import { buildPayrollReport, floorQuarter } from '../lib/payroll-report';
...
test('payroll report floors combined multi-location hours once before the 88 hour split',()=>{
  assert.equal(floorQuarter(93.35),93.25);
...
test('payroll report applies cash, cap, and hold rules after quarter-hour rounding',()=>{
  ...
  assert.equal(buildPayrollReport(punches,[{employee_name:'Test Employee',rule_type:'CASH_ONLY'}],'2026-06-30')[0].cash_hours,100);
  ...
  assert.equal(capped.cheque_hours,48);assert.equal(capped.cash_hours,52);
