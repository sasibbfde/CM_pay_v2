...
import { buildPayrollReport, roundUpHour } from '../lib/payroll-report';
...
test('payroll report rounds combined multi-location hours up to the next whole hour',()=>{
  assert.equal(roundUpHour(73),73);
  assert.equal(roundUpHour(73.4),74);
  assert.equal(roundUpHour(73.9),74);
  ...
  assert.equal(rows[0].rounded_hours,94);assert.equal(rows[0].cheque_hours,88);assert.equal(rows[0].cash_hours,6);
...
test('payroll report applies cash, cap, and hold rules after whole-hour rounding',()=>{
  const punches=[punch({hours:100.2,payroll_hours:100.2,gross_hours:101,break_minutes:48})];
  assert.equal(buildPayrollReport(punches,[{employee_name:'Test Employee',rule_type:'CASH_ONLY'}],'2026-06-30')[0].cash_hours,101);
  ...
  assert.equal(capped.cheque_hours,48);assert.equal(capped.cash_hours,53);
