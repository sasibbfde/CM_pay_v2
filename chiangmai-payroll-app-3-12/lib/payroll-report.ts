...
export const floorQuarter=(value:number)=>Math.floor((value+1e-8)*4)/4;

export type PayrollReportRow={
  employee_id:string; employee_name:string; locations:string[]; roles:string[]; location_hours:Record<string,number>;
...
export function buildPayrollReport(punches:Punch[],rules:EmployeeRule[],periodEnd:string):PayrollReportRow[]{
  ...
  return [...grouped.entries()].map(([employeeId,items])=>{
    const first=items[0];const locations=new Set<string>();const roles=new Set<string>();const locationHours:Record<string,number>={};
    ...
    for(const punch of items){
      ...
      const location=punch.location||'Unknown';locations.add(location);locationHours[location]=(locationHours[location]||0)+hours;
      if(punch.role||punch.department)roles.add(punch.role||punch.department||'');
    }
    const rounded=floorQuarter(payable);const wage=payable>0?weightedWage/payable:Number(first.wage||0);const rule=ruleFor(employeeId,first.employee_name,rules,periodEnd);
    ...
    return {employee_id:employeeId,employee_name:first.employee_name,locations:[...locations].sort(),roles:[...roles].filter(Boolean).sort(),
      location_hours:Object.fromEntries(Object.entries(locationHours).map(([key,value])=>[key,round2(value)])),wage:round2(wage),cash_wage:round2(Number(first.cash_wage||0)||wage),rule_type:rule?.rule_type||'STANDARD',rule_value:Number(rule?.rule_value||0),cheque_cap:cap,gross_hours:round2(gross),break_hours:round2(totalBreak),
