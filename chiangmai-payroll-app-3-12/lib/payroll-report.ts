...
export const roundUpHour=(value:number)=>Math.ceil(value-1e-8);
...
export function buildPayrollReport(punches:Punch[],rules:EmployeeRule[],periodEnd:string):PayrollReportRow[]{
  ...
  return [...grouped.entries()].map(([employeeId,items])=>{
    ...
    const rounded=roundUpHour(payable);const wage=payable>0?weightedWage/payable:Number(first.wage||0);const rule=ruleFor(employeeId,first.employee_name,rules,periodEnd);
