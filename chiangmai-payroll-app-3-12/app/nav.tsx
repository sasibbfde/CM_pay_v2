      ...
      <div style={{position:'relative',flexShrink:0,marginRight:6}}>
        ...
        {alertsOpen&&(
          <div style={{position:'absolute',right:0,top:34,width:360,maxHeight:420,overflowY:'auto',background:'var(--surface)',border:'1px solid var(--border2)',borderRadius:10,boxShadow:'var(--shadow-lg)',padding:10,zIndex:200}}>
            ...
            {alerts.length===0 ? <div style={{fontSize:11,color:'#6b7280',padding:12,textAlign:'center'}}>No overnight, 14h+, or Web punch-source alerts in the current month to date.</div> :
              alerts.map(alert=>(
            ...
            <div style={{fontSize:10,color:'#4b5563',paddingTop:8,lineHeight:1.35}}>Showing {alerts.length} alert{alerts.length===1?'':'s'} from the current month to date. Alerts include overnight punches, 14h+ days, and Web punch sources. Alerts are saved in audit history and do not change payroll totals.</div>
          </div>
        )}
      </div>
