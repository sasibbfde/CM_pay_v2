...
import { payrollAssignmentRows, payrollAssignmentView, payrollLocationView, summarizePayrollReport } from '@/lib/payroll-report';
import { ontarioHolidayLabel } from '@/lib/ontario-holidays';
import { isWageChangeLabel, wageChangeSourceText } from '@/lib/payroll-change-labels';

const green='FF087866',light='FFD7F0EC',orange='FFFFE0B2',yellow='FFFFF176',red='FFFFCDD2';
const cyan='FFDFFBFF',purple='FFEDE9FE',wageGreen='FFD1FAE5',manualWageYellow='FFFEF3C7',warningRed='FFFFE4E6';
...
function wageChangeNote(row:any){const source=wageChangeSourceText(row.wage_change_source);return row.wage_change_note ? `${source||'Wage change'}: ${row.wage_change_note}` : source;}
function multiLocationLabel(row:any){return row.locations?.length>1?'MULTI-LOCATION — split by worked location':'';}
function originalLocations(row:any){return row.all_locations || row.locations || [];}
function assignmentLocation(row:any){return row.display_location || row.locations?.[0] || '';}
function multiLocationPayableBreakdown(row:any){
  const locations=originalLocations(row);
  const total=locations.reduce((sum:number,location:string)=>sum+Number(row.location_hours?.[location]||0),0);
  const parts=locations.map((location:string)=>`${location}: ${(Number(row.location_hours?.[location]||0)).toFixed(2)}h`).join(' · ');
  return { total, parts };
}
function assignmentMultiLocationLabel(row:any){const locations=originalLocations(row);if(locations.length<=1)return '';const {total,parts}=multiLocationPayableBreakdown(row);return `MULTI-LOCATION — total ${total.toFixed(2)}h · ${parts}`;}
function locationHourBreakdown(row:any){
  const locations=originalLocations(row);
  return locations.length>1
    ? locations.map((name:string)=>`${name}: actual ${(Number(row.location_gross_hours?.[name]||0)).toFixed(2)}h / payable ${(Number(row.location_hours?.[name]||0)).toFixed(2)}h${row.location_holiday_hours?.[name]?` / holiday ${(Number(row.location_holiday_hours[name]||0)).toFixed(2)}h`:''}`).join(' · ')
    : '';
}
function applyChangeHighlights(excelRow:ExcelJS.Row,row:any,columns:{employee?:number;role?:number;wage?:number;labels?:number;notes?:number}){
  ...
  const wageChange=labels.some(isWageChangeLabel);
  const manualWage=row.wage_change_source==='manual';
  ...
  if(wageChange&&columns.wage){
    excelRow.getCell(columns.wage).fill={type:'pattern',pattern:'solid',fgColor:{argb:manualWage?manualWageYellow:wageGreen}};
    excelRow.getCell(columns.wage).font={bold:true,color:{argb:manualWage?'FFB45309':'FF047857'}};
  }
  ...
  for(const column of [columns.labels,columns.notes].filter(Boolean) as number[]){
    excelRow.getCell(column).fill={type:'pattern',pattern:'solid',fgColor:{argb:over14?warningRed:labels.includes('NEW')||multi?cyan:wageChange?(manualWage?manualWageYellow:wageGreen):purple}};
    excelRow.getCell(column).font={bold:true,color:{argb:over14?'FFB91C1C':labels.includes('NEW')||multi?'FF0E7490':wageChange?(manualWage?'FFB45309':'FF047857'):'FF6D28D9'}};
  }
}
function addMerged(workbook:ExcelJS.Workbook,rows:any[],start:string,end:string){
  ...
  sheet.mergeCells('A2:AA2');sheet.getCell('A2').value='Payroll authority: completed 7shifts punches only. Payable total still reconciles to 7shifts; Ontario public-holiday hours are separated. Multi-location employees are split into one row per worked location and labelled MULTI-LOCATION, because separate location payrolls/cheques are run. Holiday pay is capped to 8h and consumes the cheque cap, so regular cheque hours are reduced before excess hours move to cash. NEW, 7SHIFTS WAGE ↑, MANUAL WAGE, POSITION CHANGED, MULTI-LOCATION, and OVER 14.2H labels are highlighted for payroll review.';sheet.getCell('A2').font={italic:true,color:{argb:'FF666666'}};
  ...
  rows.forEach((row,index)=>{const multiLabel=assignmentMultiLocationLabel(row);const labelNotes=[(row.employee_labels||[]).length?`Labels: ${(row.employee_labels||[]).join(', ')}`:'',multiLabel,wageChangeNote(row),row.detail_change_note,row.notes].filter(Boolean).join('; ');const excelRow=sheet.addRow([index+1,row.employee_name,assignmentLocation(row)||row.locations.join('; '),row.roles.join(', '),row.wage,row.gross_hours,row.break_hours,row.unpaid_break_hours,row.payable_hours,row.regular_payable_hours,row.holiday_hours,null,null,null,null,null,row.holiday_pay,null,row.status,labelNotes,row.rule_type,row.cheque_cap,row.rule_value,row.cash_wage,changeLabels(row),multiLabel,locationHourBreakdown(row)]);const n=excelRow.number;
    ...
    if(originalLocations(row).length>1){excelRow.getCell(26).fill={type:'pattern',pattern:'solid',fgColor:{argb:cyan}};excelRow.getCell(27).fill={type:'pattern',pattern:'solid',fgColor:{argb:cyan}};excelRow.getCell(26).font={bold:true,color:{argb:'FF0E7490'}};}
