...
function ManagementLocationMatrix({locations}:{locations:ManagementLocation[]}){const firstDays=locations[0]?.days||[];const moneyCell=(value:number,color:string)=><span style={{color,fontWeight:600}}>{value?cadFull(value):'—'}</span>;const pct=(cost:number,sales:number)=>sales?`${(cost/sales*100).toFixed(1)}%`:'—';const groupCell=(group:{hours:number;cost:number},sales:number,color:string)=><div><div style={{color,fontWeight:650}}>{pct(group.cost,sales)}</div><div style={{fontSize:10,color:'#9ca3af'}}>{group.hours.toFixed(2)} payable h</div><div style={{fontSize:10,color:'#6b7280'}}>{cadFull(group.cost)}</div></div>;const locationTotals=(days:ManagementDay[])=>days.reduce((sum,day)=>({date:sum.date||day.date,actualSales:sum.actualSales+day.actualSales,projectedSales:sum.projectedSales+day.projectedSales,hours:sum.hours+day.hours,cost:sum.cost+day.cost,boh:{hours:sum.boh.hours+day.boh.hours,cost:sum.boh.cost+day.boh.cost},foh:{hours:sum.foh.hours+day.foh.hours,cost:sum.foh.cost+day.foh.cost},managers:{hours:sum.managers.hours+day.managers.hours,cost:sum.managers.cost+day.managers.cost}}),{date:'total',actualSales:0,projectedSales:0,hours:0,cost:0,boh:{hours:0,cost:0},foh:{hours:0,cost:0},managers:{hours:0,cost:0}});const allTotals=locationTotals(locations.flatMap(location=>location.days));const locationStyle:React.CSSProperties={padding:'10px 12px',position:'sticky',left:0,background:'#131720',zIndex:2,fontWeight:700,color:'#f9fafb',minWidth:180,borderRight:'1px solid rgba(255,255,255,.07)'};const metricStyle:React.CSSProperties={padding:'10px 12px',position:'sticky',left:180,background:'#131720',zIndex:2,fontWeight:600,color:'#e5e7eb',minWidth:155,borderRight:'1px solid rgba(255,255,255,.07)'};const headLocationStyle:React.CSSProperties={...locationStyle,fontSize:9,color:'#6b7280',textTransform:'uppercase',background:'#10141d'};const headMetricStyle:React.CSSProperties={...metricStyle,fontSize:9,color:'#6b7280',textTransform:'uppercase',background:'#10141d'};const cellStyle:React.CSSProperties={padding:'10px 12px',textAlign:'center',borderLeft:'1px solid rgba(255,255,255,.05)',minWidth:122};const metricRows=[['Actual Sales','actualSales','#34d399'],['Projected Sales','projectedSales','#9ca3af'],['Total Labour','cost','#a78bfa'],['Labour %','labourPct','#fbbf24'],['Payable Hours','hours','#22d3ee'],['Back of House','boh','#f97316'],['Front of House','foh','#22d3ee'],['Managers','managers','#a78bfa']] as const;const renderValue=(day:ManagementDay,key:typeof metricRows[number][1],color:string)=>{if(key==='actualSales'||key==='projectedSales'||key==='cost')return moneyCell(Number(day[key]),color);if(key==='labourPct')return <span style={{color:day.actualSales&&day.cost/day.actualSales>.35?'#f87171':'#fbbf24',fontWeight:700}}>{pct(day.cost,day.actualSales)}</span>;if(key==='hours')return <span style={{color,fontWeight:650}}>{day.hours.toFixed(2)}h</span>;return groupCell(day[key],day.actualSales,color);};const renderTotal=(days:ManagementDay[],key:typeof metricRows[number][1],color:string)=>renderValue(locationTotals(days),key,color);return <div style={{background:'#131720',border:'1px solid rgba(255,255,255,.07)',borderRadius:12,overflow:'hidden',marginBottom:20}}><div style={{padding:'12px 16px',borderBottom:'1px solid rgba(255,255,255,.07)'}}><div style={{fontSize:13,fontWeight:700,color:'#f9fafb'}}>Management Daily Labour Matrix by Location</div><div style={{fontSize:10,color:'#6b7280',marginTop:2}}>Single view by store · sales, labour cost, payable hours, and BOH/FOH/Managers · scroll horizontally for longer periods</div></div><div style={{overflowX:'auto'}}><table style={{borderCollapse:'collapse',fontSize:11,minWidth:'100%'}}><thead><tr style={{background:'rgba(0,0,0,.25)'}}><th style={headLocationStyle}>Location</th><th style={headMetricStyle}>Metric</th>{firstDays.map(day=><th key={day.date} style={{...cellStyle,color:'#9ca3af'}}><div>{new Date(`${day.date}T12:00:00`).toLocaleDateString('en-CA',{weekday:'short'})}</div><div style={{fontSize:9,color:'#6b7280'}}>{day.date.slice(5)}</div></th>)}<th style={{...cellStyle,color:'#22d3ee'}}>Period Total</th></tr></thead><tbody>{locations.map(location=>metricRows.map(([label,key,color],index)=><tr key={`${location.location}-${key}`} style={{borderTop:index===0?'2px solid rgba(255,255,255,.12)':'1px solid rgba(255,255,255,.05)'}}>{index===0&&<td rowSpan={metricRows.length} style={{...locationStyle,verticalAlign:'top'}}><div>{location.location}</div><div style={{fontSize:10,color:'#6b7280',fontWeight:500,marginTop:4}}>{locationTotals(location.days).hours.toFixed(1)}h · {cadFull(locationTotals(location.days).cost)}</div></td>}<td style={{...metricStyle,color:key==='boh'||key==='foh'||key==='managers'?color:'#e5e7eb'}}>{label}{(key==='boh'||key==='foh'||key==='managers')&&<div style={{fontSize:9,color:'#6b7280',fontWeight:400}}>% · payable hours · cost</div>}</td>{location.days.map(day=><td key={day.date} style={cellStyle}>{renderValue(day,key,color)}</td>)}<td style={{...cellStyle,fontWeight:700}}>{renderTotal(location.days,key,color)}</td></tr>))}<tr style={{borderTop:'3px solid rgba(34,211,238,.25)',background:'rgba(34,211,238,.04)'}}><td rowSpan={metricRows.length} style={{...locationStyle,background:'#101b24',color:'#22d3ee',verticalAlign:'top'}}><div>All Locations</div><div style={{fontSize:10,color:'#9ca3af',fontWeight:500,marginTop:4}}>{allTotals.hours.toFixed(1)}h · {cadFull(allTotals.cost)}</div></td><td style={{...metricStyle,background:'#101b24'}}>Actual Sales</td>{firstDays.map(day=>{const total=locationTotals(locations.map(location=>location.days.find(item=>item.date===day.date)).filter(Boolean) as ManagementDay[]);return <td key={day.date} style={cellStyle}>{moneyCell(total.actualSales,'#34d399')}</td>;})}<td style={cellStyle}>{moneyCell(allTotals.actualSales,'#34d399')}</td></tr>{metricRows.slice(1).map(([label,key,color])=><tr key={`total-${key}`} style={{borderTop:'1px solid rgba(255,255,255,.05)',background:'rgba(34,211,238,.04)'}}><td style={{...metricStyle,background:'#101b24',color:key==='boh'||key==='foh'||key==='managers'?color:'#e5e7eb'}}>{label}{(key==='boh'||key==='foh'||key==='managers')&&<div style={{fontSize:9,color:'#6b7280',fontWeight:400}}>% · payable hours · cost</div>}</td>{firstDays.map(day=>{const total=locationTotals(locations.map(location=>location.days.find(item=>item.date===day.date)).filter(Boolean) as ManagementDay[]);return <td key={day.date} style={cellStyle}>{renderValue(total,key,color)}</td>;})}<td style={{...cellStyle,fontWeight:700}}>{renderValue(allTotals,key,color)}</td></tr>)}</tbody></table></div></div>}
