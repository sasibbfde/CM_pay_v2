...
type ManagementLocation={location:string;days:ManagementDay[]};
...
function ManagementMatrix({days}:{days:ManagementDay[]}){const totals=days.reduce((sum,day)=>({date:sum.date||day.date,actualSales:sum.actualSales+day.actualSales,projectedSales:sum.projectedSales+day.projectedSales,hours:sum.hours+day.hours,cost:sum.cost+day.cost,boh:{hours:sum.boh.hours+day.boh.hours,cost:sum.boh.cost+day.boh.cost},foh:{hours:sum.foh.hours+day.foh.hours,cost:sum.foh.cost+day.foh.cost},managers:{hours:sum.managers.hours+day.managers.hours,cost:sum.managers.cost+day.managers.cost}}),{date:'total',actualSales:0,projectedSales:0,hours:0,cost:0,boh:{hours:0,cost:0},foh:{hours:0,cost:0},managers:{hours:0,cost:0}});const moneyCell=(value:number,color:string)=><span style={{color,fontWeight:600}}>{value?cadFull(value):'—'}</span>;const groupCell=(group:{hours:number;cost:number},sales:number,color:string)=><div><div style={{color,fontWeight:650}}>{sales?`${(group.cost/sales*100).toFixed(1)}%`:'—'}</div><div style={{fontSize:10,color:'#9ca3af'}}>{group.hours.toFixed(2)} payable h</div><div style={{fontSize:10,color:'#6b7280'}}>{cadFull(group.cost)}</div></div>;const labelStyle:React.CSSProperties={padding:'9px 12px',position:'sticky',left:0,background:'#131720',zIndex:1,fontWeight:600,color:'#e5e7eb',minWidth:145};const cellStyle:React.CSSProperties={padding:'9px 12px',textAlign:'center',borderLeft:'1px solid rgba(255,255,255,.05)',minWidth:112};return <div style={{background:'#131720',border:'1px solid rgba(255,255,255,.07)',borderRadius:12,overflow:'hidden',marginBottom:20}}><div style={{padding:'12px 16px',borderBottom:'1px solid rgba(255,255,255,.07)'}}><div style={{fontSize:13,fontWeight:700,color:'#f9fafb'}}>Management Daily Labour Matrix</div><div style={{fontSize:10,color:'#6b7280',marginTop:2}}>Sales, labour cost and payable hours by department · scroll horizontally for longer periods</div></div><div style={{overflowX:'auto'}}><table style={{borderCollapse:'collapse',fontSize:11,minWidth:'100%'}}><thead><tr style={{background:'rgba(0,0,0,.2)'}}><th style={{...labelStyle,fontSize:9,color:'#6b7280',textTransform:'uppercase'}}>Metric</th>{days.map(day=><th key={day.date} style={{...cellStyle,color:'#9ca3af'}}><div>{new Date(`${day.date}T12:00:00`).toLocaleDateString('en-CA',{weekday:'short'})}</div><div style={{fontSize:9,color:'#6b7280'}}>{day.date.slice(5)}</div></th>)}<th style={{...cellStyle,color:'#22d3ee'}}>Period Total</th></tr></thead><tbody><tr style={{borderTop:'1px solid rgba(255,255,255,.05)'}}><td style={labelStyle}>Actual Sales</td>{days.map(day=><td key={day.date} style={cellStyle}>{moneyCell(day.actualSales,'#34d399')}</td>)}<td style={cellStyle}>{moneyCell(totals.actualSales,'#34d399')}</td></tr><tr style={{borderTop:'1px solid rgba(255,255,255,.05)'}}><td style={labelStyle}>Projected Sales</td>{days.map(day=><td key={day.date} style={cellStyle}>{moneyCell(day.projectedSales,'#9ca3af')}</td>)}<td style={cellStyle}>{moneyCell(totals.projectedSales,'#9ca3af')}</td></tr><tr style={{borderTop:'1px solid rgba(255,255,255,.05)'}}><td style={labelStyle}>Total Labour</td>{days.map(day=><td key={day.date} style={cellStyle}>{moneyCell(day.cost,'#a78bfa')}</td>)}<td style={cellStyle}>{moneyCell(totals.cost,'#a78bfa')}</td></tr><tr style={{borderTop:'1px solid rgba(255,255,255,.05)'}}><td style={labelStyle}>Labour %</td>{days.map(day=><td key={day.date} style={{...cellStyle,color:day.actualSales&&day.cost/day.actualSales>.35?'#f87171':'#fbbf24',fontWeight:700}}>{day.actualSales?`${(day.cost/day.actualSales*100).toFixed(1)}%`:'—'}</td>)}<td style={{...cellStyle,color:'#fbbf24',fontWeight:700}}>{totals.actualSales?`${(totals.cost/totals.actualSales*100).toFixed(1)}%`:'—'}</td></tr><tr style={{borderTop:'1px solid rgba(255,255,255,.05)'}}><td style={labelStyle}>Payable Hours</td>{days.map(day=><td key={day.date} style={{...cellStyle,color:'#22d3ee'}}>{day.hours.toFixed(2)}h</td>)}<td style={{...cellStyle,color:'#22d3ee',fontWeight:700}}>{totals.hours.toFixed(2)}h</td></tr>{([['Back of House','boh','#f97316'],['Front of House','foh','#22d3ee'],['Managers','managers','#a78bfa']] as const).map(([label,key,color])=><tr key={key} style={{borderTop:'1px solid rgba(255,255,255,.06)'}}><td style={{...labelStyle,color}}>{label}<div style={{fontSize:9,color:'#6b7280',fontWeight:400}}>% · payable hours · cost</div></td>{days.map(day=><td key={day.date} style={cellStyle}>{groupCell(day[key],day.actualSales,color)}</td>)}<td style={cellStyle}>{groupCell(totals[key],totals.actualSales,color)}</td></tr>)}</tbody></table></div></div>}
function ManagementLocationMatrix({locations}:{locations:ManagementLocation[]}) {
  const [viewMode,setViewMode]=useState<'summary'|'department'|'full'>('department');
  const [expanded,setExpanded]=useState<Record<string,boolean>>({});
  const firstDays=locations[0]?.days||[];
  const emptyDay:ManagementDay={date:'total',actualSales:0,projectedSales:0,hours:0,cost:0,boh:{hours:0,cost:0},foh:{hours:0,cost:0},managers:{hours:0,cost:0}};
  const locationTotals=(days:ManagementDay[])=>days.reduce((sum,day)=>({date:sum.date||day.date,actualSales:sum.actualSales+day.actualSales,projectedSales:sum.projectedSales+day.projectedSales,hours:sum.hours+day.hours,cost:sum.cost+day.cost,boh:{hours:sum.boh.hours+day.boh.hours,cost:sum.boh.cost+day.boh.cost},foh:{hours:sum.foh.hours+day.foh.hours,cost:sum.foh.cost+day.foh.cost},managers:{hours:sum.managers.hours+day.managers.hours,cost:sum.managers.cost+day.managers.cost}}),emptyDay);
  const allTotals=locationTotals(locations.flatMap(location=>location.days));
  const dayTotal=(date:string)=>locationTotals(locations.map(location=>location.days.find(day=>day.date===date)).filter(Boolean) as ManagementDay[]);
  const moneyCell=(value:number,color:string)=><span style={{color,fontWeight:650}}>{value?cadFull(value):'—'}</span>;
  const pct=(cost:number,sales:number)=>sales?`${(cost/sales*100).toFixed(1)}%`:'—';
  const groupCell=(group:{hours:number;cost:number},sales:number,color:string)=><div><div style={{color,fontWeight:750}}>{pct(group.cost,sales)}</div><div style={{fontSize:10,color:'#9ca3af',whiteSpace:'nowrap'}}>{group.hours.toFixed(2)} payable h</div><div style={{fontSize:10,color:'#6b7280'}}>{cadFull(group.cost)}</div></div>;
  const baseRows=[
    ['Actual Sales','actualSales','#34d399','summary'],
    ['Projected Sales','projectedSales','#9ca3af','full'],
    ['Total Labour','cost','#a78bfa','summary'],
    ['Labour %','labourPct','#fbbf24','summary'],
    ['Payable Hours','hours','#22d3ee','summary'],
    ['Back of House','boh','#f97316','department'],
    ['Front of House','foh','#22d3ee','department'],
    ['Managers','managers','#a78bfa','department'],
  ] as const;
  type MetricKey=typeof baseRows[number][1];
  const visibleRows=baseRows.filter(([,key,,level])=>viewMode==='full'||level==='summary'||(viewMode==='department'&&level==='department')).filter(([,key])=>viewMode!=='summary'||!['boh','foh','managers'].includes(key));
  const renderValue=(day:ManagementDay,key:MetricKey,color:string)=>{
    if(key==='actualSales'||key==='projectedSales'||key==='cost')return moneyCell(Number(day[key]),color);
    if(key==='labourPct')return <span style={{color:day.actualSales&&day.cost/day.actualSales>.35?'#f87171':'#fbbf24',fontWeight:800}}>{pct(day.cost,day.actualSales)}</span>;
    if(key==='hours')return <span style={{color,fontWeight:750}}>{day.hours.toFixed(2)}h</span>;
    return groupCell(day[key],day.actualSales,color);
  };
  const locationStyle:React.CSSProperties={padding:'10px 12px',position:'sticky',left:0,background:'#131720',zIndex:2,fontWeight:800,color:'#f9fafb',minWidth:190,borderRight:'1px solid rgba(255,255,255,.08)'};
  const metricStyle:React.CSSProperties={padding:'10px 12px',position:'sticky',left:190,background:'#131720',zIndex:2,fontWeight:650,color:'#e5e7eb',minWidth:155,borderRight:'1px solid rgba(255,255,255,.08)'};
  const cellStyle:React.CSSProperties={padding:'10px 12px',textAlign:'center',borderLeft:'1px solid rgba(255,255,255,.05)',minWidth:122};
  const modeButton=(mode:typeof viewMode,label:string)=><button type="button" onClick={()=>setViewMode(mode)} style={{background:viewMode===mode?'rgba(34,211,238,.16)':'rgba(255,255,255,.04)',border:`1px solid ${viewMode===mode?'rgba(34,211,238,.45)':'rgba(255,255,255,.08)'}`,color:viewMode===mode?'#22d3ee':'#9ca3af',borderRadius:999,padding:'6px 10px',fontSize:11,fontWeight:700,cursor:'pointer'}}>{label}</button>;
  const renderLocationRows=(locationName:string,days:ManagementDay[],isTotal=false)=>{
    const totals=locationTotals(days);
    const isOpen=isTotal||viewMode==='full'||expanded[locationName]!==false;
    const rows=isOpen?visibleRows:visibleRows.filter(([,key])=>['actualSales','cost','labourPct','hours'].includes(key));
    return rows.map(([label,key,color],index)=><tr key={`${locationName}-${key}`} style={{borderTop:index===0?(isTotal?'3px solid rgba(34,211,238,.35)':'2px solid rgba(255,255,255,.12)'):'1px solid rgba(255,255,255,.05)',background:isTotal?'rgba(34,211,238,.045)':'transparent'}}>
      {index===0&&<td rowSpan={rows.length} style={{...locationStyle,background:isTotal?'#101b24':'#131720',color:isTotal?'#22d3ee':'#f9fafb',verticalAlign:'top'}}>
        <button type="button" onClick={()=>!isTotal&&setExpanded(prev=>({...prev,[locationName]:!isOpen}))} style={{all:'unset',display:'flex',gap:8,alignItems:'center',cursor:isTotal?'default':'pointer'}}>
          <span style={{color:isTotal?'#22d3ee':'#9ca3af'}}>{isTotal?'◆':isOpen?'▼':'▶'}</span>
          <span>{locationName}</span>
        </button>
        <div style={{fontSize:10,color:'#9ca3af',fontWeight:500,marginTop:5,lineHeight:1.45}}>{totals.hours.toFixed(1)} payable h<br />{cadFull(totals.cost)} labour</div>
      </td>}
      <td style={{...metricStyle,background:isTotal?'#101b24':'#131720',color:key==='boh'||key==='foh'||key==='managers'?color:'#e5e7eb'}}>
        {label}
        {(key==='boh'||key==='foh'||key==='managers')&&<div style={{fontSize:9,color:'#6b7280',fontWeight:400}}>% · payable hours · cost</div>}
      </td>
      {days.map(day=><td key={day.date} style={cellStyle}>{renderValue(day,key,color)}</td>)}
      <td style={{...cellStyle,fontWeight:800,background:isTotal?'rgba(34,211,238,.035)':'transparent'}}>{renderValue(totals,key,color)}</td>
    </tr>);
  };
  return <div style={{background:'#131720',border:'1px solid rgba(255,255,255,.07)',borderRadius:12,overflow:'hidden',marginBottom:20}}>
    <div style={{padding:'12px 16px',borderBottom:'1px solid rgba(255,255,255,.07)',display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap'}}>
      <div>
        <div style={{fontSize:13,fontWeight:800,color:'#f9fafb'}}>Management Daily Labour Matrix by Location</div>
        <div style={{fontSize:10,color:'#6b7280',marginTop:2}}>All stores in one view · sticky Location/Metric columns · expand a store for department detail</div>
      </div>
      <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
        {modeButton('summary','Summary')}
        {modeButton('department','Department')}
        {modeButton('full','Full Detail')}
      </div>
    </div>
      ...
      <table style={{borderCollapse:'collapse',fontSize:11,minWidth:'100%'}}>
        <thead>
          <tr style={{background:'rgba(0,0,0,.25)'}}>
            <th style={{...locationStyle,fontSize:9,color:'#6b7280',textTransform:'uppercase',background:'#10141d'}}>Location</th>
            <th style={{...metricStyle,fontSize:9,color:'#6b7280',textTransform:'uppercase',background:'#10141d'}}>Metric</th>
            {firstDays.map(day=><th key={day.date} style={{...cellStyle,color:'#9ca3af'}}><div>{new Date(`${day.date}T12:00:00`).toLocaleDateString('en-CA',{weekday:'short'})}</div><div style={{fontSize:9,color:'#6b7280'}}>{day.date.slice(5)}</div></th>)}
            <th style={{...cellStyle,color:'#22d3ee'}}>Period Total</th>
          </tr>
        </thead>
        <tbody>
          {renderLocationRows('All Locations',firstDays.map(day=>dayTotal(day.date)),true)}
          {locations.map(location=>renderLocationRows(location.location,location.days))}
        </tbody>
      </table>
    </div>
  </div>;
...
export default function LabourPage() {
  ...
  const managementByLocation=useMemo(()=>{const scopedLocations=locationFilter==='ALL'?locations:[locationFilter];return scopedLocations.filter(Boolean).map(location=>({location,days:managementDays.map(date=>{const groups=dailyLabourGroups.filter(row=>row.date===date&&row.location===location);const salesRows=dailySales.filter(row=>row.sale_date===date&&row.location===location);const rawByGroup=(group:string)=>groups.filter(row=>row.group===group).reduce((sum,row)=>({hours:sum.hours+row.hours,cost:sum.cost+row.cost}),{hours:0,cost:0});const actualSales=salesRows.reduce((sum,row)=>sum+Number(row.net_sales||0),0);const projectedSales=salesRows.reduce((sum,row)=>sum+Number(row.projected_sales||0),0);const hours=groups.reduce((sum,row)=>sum+row.hours,0);const cost=groups.reduce((sum,row)=>sum+row.cost,0);return{date,actualSales,projectedSales,hours,cost,boh:rawByGroup('Back of House'),foh:rawByGroup('Front of House'),managers:rawByGroup('Managers')};})})).filter(item=>item.days.some(day=>day.actualSales||day.projectedSales||day.hours||day.cost));},[locationFilter,locations,managementDays,dailyLabourGroups,dailySales]);
  ...
  return(
    <div style={{background:'#0a0c10',minHeight:'100vh',color:'#e5e7eb',fontFamily:'Inter,sans-serif',padding:'24px 32px'}}>
      ...
      {locationFilter==='ALL'&&managementByLocation.length>0?<ManagementLocationMatrix locations={managementByLocation}/>:<ManagementMatrix days={management}/>}
