...
export function calculatePayroll(
  ...
  for (const [key, items] of grouped) {
    ...
    const actual = items.reduce((s, p) => s + getPunchHours(p), 0);
    ...
    if (rule) {
      ...
      if (rule.rule_type === 'CASH_ONLY') {
        ...
        cashHours = actual;
        payrollAmount = 0;
        cashAmount = actual * wage;
      }

      if (
        rule.rule_type === 'PAYROLL_HOURS_CAP' ||
        rule.rule_type === 'COMBINED_LOCATION_CAP'
      ) {
        const cap = Number(rule.rule_value || 0);
        payrollHours = Math.min(actual, cap);
        cashHours = Math.max(actual - cap, 0);
        payrollAmount = payrollHours * wage;
        cashAmount = cashHours * wage;
      }
    ...
    rows.push({
      ...
      payroll_hours: round(payrollHours),
      cash_hours: round(cashHours),
      wage,
      payroll_amount: round(payrollAmount),
      cash_amount: round(cashAmount),
