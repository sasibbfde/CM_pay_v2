...
export async function GET(
  ...
  try {
    ...
    // Ensure payroll_hours is populated (backward compat for older punches)
    const punches = (data || []).map(p => ({
      ...p,
      // Use stored payroll_hours if available, fall back to hours
    ...
    // Monthly summary using PAYROLL hours (not raw clock diff)
    const totalPayrollHours = punches
      .filter(p => p.clocked_out) // exclude ongoing shifts
