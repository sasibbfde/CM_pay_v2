...
/** Fetch ALL rows from a Supabase table using range pagination — bypasses the 1000 row default limit */
...
export async function GET(req: NextRequest) {
  ...
  if (!hasSupabaseEnv()) {
    return NextResponse.json({
      ...
      summary: { totalHours:0, payrollHours:0, cashHours:0, payrollAmount:0, cashAmount:0, exceptions:0 },
      rows: [], monthly: [], yearly: []
    });
  }
  ...
  try {
    ...
    return NextResponse.json({
      ...
      monthly,
      yearly,
      counts: { punches: punches.length, rules: rules.length }
    });
