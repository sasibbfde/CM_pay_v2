...
async function runSync(body: any) {
  ...
  try {
    ...
    for (let i = 0; i < userRows.length; i += BATCH) {
      ...
      // Use raw SQL upsert that preserves existing wage/location/dept/role
      // 7shifts API rarely returns these on user object — we populate from punches+mastersheet
      const safeRows = batch.map((r: any) => ({
        employee_id:          r.employee_id,
        seven_shifts_user_id: r.seven_shifts_user_id,
        ...
        // Only include wage/location/dept/role if 7shifts has a real value
        ...(r.wage > 0        ? { wage: r.wage }           : {}),
      ...
      const { error } = await supabase.from('employees')
        .upsert(safeRows, {
          onConflict: 'seven_shifts_user_id',
    ...
    try {
      ...
      for (const p of punchList) {
        ...
        // Gross hours = raw clock diff (no breaks deducted)
        const grossHours = (clockIn && clockOut)
          ? Math.round((new Date(clockOut).getTime() - new Date(clockIn).getTime()) / 36000) / 100
          : hours;
        // Break minutes = difference between gross and payroll hours
        const breakMinutes = hours > 0 && grossHours > hours
          ? Math.round((grossHours - hours) * 60)
          : 0;
        // Break from 7shifts fields
        const breakMins7s = Number(p.break || p.break_time || p.unpaid_break_time || breakMinutes);
        ...
        punchMap.set(punchId, {
          ...
          hours,          // payroll-approved hours (break already deducted by 7shifts)
          payroll_hours:  hours,
          gross_hours:    grossHours,
          break_minutes:  breakMins7s,
