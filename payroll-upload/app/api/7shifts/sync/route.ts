...
// fetchLocations() will extend this at runtime with live data.
const STATIC_LOCATION_MAP: Record<string, string> = {
  ...
  '467000': 'Chiang Mai Mississauga',   // ← added
};
  ...
  try {
    ...
    try {
      ...
      const punchRows = punchList.map((p: any) => {
        ...
        const matchedUser  = userList.find((u: any) => String(u.id) === String(userId));
        const employeeName =
          p.employee_name ||
          p.user_name ||
          (matchedUser ? fullName(matchedUser) : `User ${userId || 'Unknown'}`);

        const rawWage = p.hourly_wage || p.wage || matchedUser?.hourly_wage || 0;

        // Resolve location: prefer name fields, fall back to id mapping
