...
export async function proxy(req: NextRequest) {
  ...
  if (!url || !key) return new NextResponse('Supabase authentication is not configured', { status: 503 });
  ...
  if (!authenticated && !isPublic) {
    if (req.nextUrl.pathname.startsWith('/api/')) return copyAuthState(response, NextResponse.json({ error:'Unauthorized' }, { status:401 }));
